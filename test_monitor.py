...
def test_traffic_complete_days_cache_and_paid_orders(tmp_path):
    ...
    r = asyncio.run(m.traffic(Visits(), [order(), order(), cancelled], start, end))
...
def test_traffic_missing_invalid_or_wrong_cutoff_is_not_zero(tmp_path, payload):
    ...
    class Visits:
        async def get(self, path, params):
            if payload is None: raise RuntimeError('Mercado Libre devolvió HTTP 403')
            return payload
    start = datetime(2026,9,17,tzinfo=TZ)
